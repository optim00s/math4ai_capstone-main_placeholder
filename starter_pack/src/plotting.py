...
def plot_optimizer_comparison(histories_dict, title="Optimizer Study (Digits)",
                              filename=None):
    """Plot optimizer comparison for the NN on digits."""
    fig, axes = plt.subplots(1, 2, figsize=(14, 5.5))
    ...
    for name, hist in histories_dict.items():
        ...
        epochs = range(1, len(hist['train_loss']) + 1)
        axes[0].plot(epochs, hist['train_loss'], '-', color=c, alpha=0.35, linewidth=1)
        axes[0].plot(epochs, hist['val_loss'], '-', color=c, linewidth=2.5,
                     label=f'{name} val')
    ...
    for name, hist in histories_dict.items():
        c = colors.get(name, '#000000')
        epochs = range(1, len(hist['train_acc']) + 1)
        axes[1].plot(epochs, hist['train_acc'], '-', color=c, alpha=0.35, linewidth=1)
        axes[1].plot(epochs, hist['val_acc'], '-', color=c, linewidth=2.5,
                     label=f'{name} val')
...
def plot_pca_2d(X_2d, y, filename=None):
    ...
    ax.set_title('2D PCA Projection of Digits Data',
...
def plot_pca_softmax_comparison(dims, val_accs, val_losses, filename=None):
    """Bar chart comparing softmax at different PCA dimensions."""
    ...
    axes[0].set_title('Accuracy vs PCA Dimensions', fontweight='bold')

    bars2 = axes[1].bar(x, val_losses, color=bar_color, alpha=0.85,
                        edgecolor='white', linewidth=1.5, width=0.6)
    ...
    axes[1].set_title('Loss vs PCA Dimensions', fontweight='bold')
